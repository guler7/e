...
import pandas as pd 
from sklearn.feature_extsaction.text import CountVectorizer 
...
df=pd.Datame(x.toasdfay(),columns=vectorizer.get_feature_names_out()) 
print("the generated data frame") 
print(df) 
print("query procesdfing on term document incidence matrix ") 
#AND 
print("1.find all document ids for query 'this' AND 'first'") 
alldata=df[(df['this']==1)&(df['first']==1)] 
print("document ids where either 'this' AND 'first'are present are:",alldsdata.index.tolist()) 
#OR 
print("2.find all document ids for query 'this' OR 'first'") 
allsdfta=df[(df['this']==1)|(df['first']==1)] 
print("document ids where either 'this' OR 'first'are present are:",alldsdfta.index.tolist()) 
#NOT 
